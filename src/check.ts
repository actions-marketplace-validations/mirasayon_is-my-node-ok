...
async function fetchJson(obj: (typeof STORE)["security"]) {
...
async function getJson(obj: (typeof STORE)["security"]) {
    return new Promise((resolve) => {
        request(obj.url, { method: "HEAD" }, (res) => {
            ...
            if (!obj.etagValue || obj.etagValue !== etag || !fs.existsSync(obj.jsonFile)) {
                obj.etagValue = etag as string;
                fs.writeFileSync(obj.etagFile, etag as string);
...
async function main(currentVersion: string, platform: string) {
    ...
    if (isEOL) {
        ...
        console.error(`Node.js ${currentVersion} is end-of-life. There are high chances of being vulnerable. Please upgrade it.`);
        return;
    }

    const vulnerabilitiesJSON = (await getJson(STORE.security)) as corejson;
